...
const icons = ['Shield', 'Target', 'BookOpen', 'Brain', 'Crosshair', 'Gift', 'ArrowRight', 'CheckCircle', 'Users', 'User'];
...
const navReplacement = \`const navigation = [
    { 
      ...
      href: '/strelnica',
      dropdown: [
        { name: 'O strelnici', href: '/strelnica' },
        { name: 'Ako to funguje', href: '/som-tu-prvykrat' },
        { name: 'Bezpečnosť', href: '/strelnica' },
...
// Since there's an existing isMegaMenu block, let's find it.
                            ...
                            <div className="flex">
                              ...
                              <div className="w-[40%] bg-zinc-950 p-6 border-l border-zinc-800/80 flex flex-col justify-center gap-4 relative overflow-hidden">
                                ...
                                <Link 
                                  ...
                                  className="bg-zinc-900 border border-zinc-800 hover:border-[var(--color-safety)]/50 p-5 rounded-xl group transition-all relative overflow-hidden flex flex-col items-start text-left"
                                >
                                ...
                                <Link 
                                  ...
                                  onClick={() => setClosedDropdowns(prev => ({ ...prev, [item.name]: true }))}
                                  className="bg-zinc-900 border border-zinc-800 hover:border-white/20 p-5 rounded-xl group transition-all relative overflow-hidden flex flex-col items-start text-left mt-2"
                                >
...
const newCTA = \`{/* Prominent CTAs */}
                <Link to="/darcekovy-poukaz" className="hidden lg:flex bg-[var(--color-safety)] text-black px-4 py-2 rounded-lg font-bold tracking-wider uppercase items-center gap-2 hover:bg-yellow-400 transition-colors shadow-[0_0_15px_rgba(251,188,5,0.2)] text-xs mr-2">
...
const oldKontaktTarget = /<Link\s*to="\/kontakt"[\s\S]*?\{t\('nav.kontakt'\)\}[\s\S]*?<\/Link>/g;
// Actually the old desktop Kontakt link looks like this:
// <Link to="/kontakt" className="bg-[var(--color-safety)] text-[var(--color-tactical)] px-6 py-2.5 rounded-lg font-display text-base font-bold tracking-widest uppercase hover:bg-white transition-all shadow-lg hidden lg:block">
