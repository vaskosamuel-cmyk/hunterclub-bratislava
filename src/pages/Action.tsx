...
import { ShieldCheck, UserCheck, Shield, ChevronRight, Gift, ChevronDown, Star, Check, X } from 'lucide-react';
...
export default function Action() {
  ...
  const packages = [
    {
      name: language === 'sk' ? "Legendary" : language === 'en' ? "Legendary" : language === 'de' ? "Legendär" : "Легендарный",
    ...
    {
      name: language === 'sk' ? "Pištoľ / Karabína 9mm" : language === 'en' ? "Pistol / Carbine 9mm" : language === 'de' ? "Pistole / Karabiner 9mm" : "Пистолет / Карабин 9мм",
    ...
    {
      name: language === 'sk' ? "Dlhé zbrane" : language === 'en' ? "Long Guns" : language === 'de' ? "Langwaffen" : "Длинноствольное оружие",
  ...
  return (
    <div className="bg-[var(--color-tactical)] min-h-screen font-sans text-white selection:bg-[#D32F2F] selection:text-white pb-20">
        ...
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          ...
          <div className="relative" onMouseEnter={() => setIsLangOpen(true)} onMouseLeave={() => setIsLangOpen(false)}>
            ...
            >
              ...
              <span className="hidden md:inline font-bold uppercase">{language}</span>
                ...
                >
                  <div className="bg-[var(--color-forest)]/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] p-2 flex flex-col min-w-[120px]">
                    <button onClick={() => handleLanguageChange('sk')} className={clsx("flex items-center gap-3 px-4 py-2 rounded-lg hover:bg-white/10 transition-colors text-left", language === 'sk' && "text-[var(--color-safety)]")}>
                      <span className="text-lg">🇸🇰</span> <span className="font-bold">SK</span>
                    </button>
                    <button onClick={() => handleLanguageChange('de')} className={clsx("flex items-center gap-3 px-4 py-2 rounded-lg hover:bg-white/10 transition-colors text-left", language === 'de' && "text-[var(--color-safety)]")}>
                      <span className="text-lg">🇩🇪</span> <span className="font-bold">DE</span>
                    </button>
                    <button onClick={() => handleLanguageChange('en')} className={clsx("flex items-center gap-3 px-4 py-2 rounded-lg hover:bg-white/10 transition-colors text-left", language === 'en' && "text-[var(--color-safety)]")}>
                      <span className="text-lg">🇬🇧</span> <span className="font-bold">EN</span>
                    </button>
                    <button onClick={() => handleLanguageChange('ru')} className={clsx("flex items-center gap-3 px-4 py-2 rounded-lg hover:bg-white/10 transition-colors text-left", language === 'ru' && "text-[var(--color-safety)]")}>
                      <span className="text-lg">🇷🇺</span> <span className="font-bold">RU</span>
                    </button>
                  </div>
                </motion.div>
              )}
            ...
            {packages.map((pkg, idx) => (
              ...
              >
                ...
                <div className="relative z-10 flex-grow flex flex-col">
                  ...
                  <div className="text-5xl md:text-6xl font-display text-[var(--color-safety)] mb-8">
                    {pkg.price}
                  </div>
      ...
      <AnimatePresence>
        {showGiftPopup && (
          <div className="fixed bottom-4 left-4 right-4 sm:bottom-6 sm:left-auto sm:right-6 z-50 sm:w-[384px]">
            <motion.div
              initial={{ opacity: 0, y: 50, scale: 0.9 }}
              animate={{ opacity: 1, y: 0, scale: 1 }}
              exit={{ opacity: 0, y: 50, scale: 0.9 }}
              className="bg-[var(--color-forest)] border border-[var(--color-safety)]/50 p-6 sm:p-8 rounded-sm shadow-[0_20px_50px_rgba(0,0,0,0.5)] relative overflow-hidden"
            >
              <div className="absolute inset-0 tactical-mosaic opacity-10 pointer-events-none"></div>
              <button 
